...
Node* removetail(Node* head) {
    ...
    head->next==null means only one element in the list
    in this both situation, we cann't do any operation .
    */

    Node* temp = head;//making a copy of head, because we don't want to change the orginal head

    while(temp->next->next!=NULL )///temp->next>next means after last node,which is null
    {
       temp= temp->next;
